...
func init() {
	...
	service := micro.NewService(
		micro.Name("go.micro.service.apigw"), // 在注册中心中的服务名称
		micro.RegisterTTL(time.Second*10),    // 声明超时时间, 避免consul不主动删掉已失去心跳的服务节点
		micro.RegisterInterval(time.Second*5),
		micro.Registry(registry),
		micro.WrapClient(ratelimit.NewClientWrapper(bRate, false)), //限流
		micro.WrapClient(hystrix.NewClientWrapper()),               //熔断
	)
	// 初始化， 解析命令行参数等
	service.Init()
	...
	// 初始化一个account服务的客户端
	userCli = userProto.NewUserService("go.micro.service.account", service.Client())
	// 初始化一个upload服务的客户端
