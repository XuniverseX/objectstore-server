...
import (
	...
	mydb "objectstore-server/service/dbproxy/conn"
)
...
func QueryUserFileMetas(username string, limit int64) (res ExecResult) {
	stmt, err := mydb.DBConn().Prepare(
		"select file_hash,file_name,file_size,upload_at," +
...
func QueryUserFileMeta(username string, filehash string) (res ExecResult) {
	stmt, err := mydb.DBConn().Prepare(
		"select file_hash,file_name,file_size,upload_at," +
			"last_update from tbl_user_file where user_name=? and file_hash=?  limit 1")
	if err != nil {
		res.Suc = false
		res.Msg = err.Error()
		return
